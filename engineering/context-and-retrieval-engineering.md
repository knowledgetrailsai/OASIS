...
Context engineering and prompt engineering are often used interchangeably, but Chapter 14 treats them as distinct disciplines operating at different scopes, and the distinction is worth taking seriously rather than treating as pedantry. Prompt engineering is about instructions — the fixed wording that tells a model what to do, what format to respond in, and what examples to follow. Context engineering is about everything else the model sees at a given decision point: retrieved evidence, conversation and workflow state, memory, tool definitions and results, applicable policy, and the token budget all of that has to fit inside. A well-engineered prompt pointed at a poorly engineered context still fails, because the model is reasoning over the wrong, incomplete, or stale evidence — which is why Chapter 14 frames context engineering as "everything the model should see at a decision point," not a stylistic add-on to prompt design.

The retrieval side of context engineering has a specific failure mode worth naming explicitly: the temptation to solve context-quality problems by making the context window bigger rather than the retrieved evidence better. Larger context windows reduce the *cost* of including more evidence, but they do not change the underlying principle Chapter 14 states directly — the goal is "the smallest sufficient and authoritative evidence set," not the largest possible context. A model given ten marginally-relevant documents to reason over performs worse, not better, than one given the two documents that actually answer the question, even when the ten would fit comfortably in the window; irrelevant context competes for the model's attention and increases the surface area for injection and contamination (see the [Security checklist's](../security/agentic-ai-threat-and-control-checklist.md#1-defense-in-depth-control-layers-ch-19-mapped-to-owasp-llm-top-10) context-layer row). This is why Chapter 14 also insists that retrieval be evaluated *separately* from generation — a system can generate excellent answers from bad retrieved evidence by accident often enough to mask a retrieval problem until it fails in production on a case the evaluation set didn't happen to cover.
