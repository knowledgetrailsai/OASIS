...
Chapter 18 built the evidence system that tells a team whether an intelligence system is good enough to ship. That system has a blind spot: an evaluation dataset covers failures a team could imagine, not a deliberately engineered attack. This chapter asks what happens when an adversary, a compromised dependency, or a user's mistake pushes the system outside every tested condition. A system nobody is attacking can still cause harm through bias, opacity, or a lack of human control — responsible AI engineering keeps that harm from being the default.
