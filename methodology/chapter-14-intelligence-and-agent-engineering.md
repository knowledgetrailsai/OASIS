...
For work that can outlive one session, treat the harness as a durable job runner. Save a run ID, checkpoint state after each meaningful step, and make restart and cancellation normal operations. A long-running run is not ready until it can resume without repeating a completed side effect.
